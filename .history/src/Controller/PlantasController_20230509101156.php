    ...
    public function nueva(Request $request,FrontManager $frontManager): Response
    {
        if(isset($_COOKIE['jwt_token'])){
            ...
            if($request->getMethod()==='POST'){
                ...
                $options_find=['headers' => ['Authorization' => 'Bearer '.$token, 'Accept'        => 'application/json'],];
                $plantaquery_params = ['nombre' => $planta_nombre];
                $planta_find_response=$frontManager->petition('GET',$options_find,$planta_find_relative_url);
                $planta_find_response_content=json_decode($planta_find_response->getBody(),true);
